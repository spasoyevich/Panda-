...
?>
<p><i>Vous êtes ici</i> : <a href="../includes/index.php">Index du forum</a> --> Enregistrement';
...
else //On est dans le cas traitement
{
    ...
    //Vérification du pseudo avec requetés envoye au base de données
    $query=$db->prepare('SELECT COUNT(*) AS nbr FROM forum_membres WHERE membre_pseudo =:pseudo');
    $query->bindValue(':pseudo',$pseudo, PDO::PARAM_STR);
    $query->execute();
    $pseudo_free=($query->fetchColumn()==0)?1:0;
    $query->CloseCursor();
    if(!$pseudo_free)//si le pseudo est déjà pris je envoie un message de erreur
    {
        $pseudo_erreur1 = "Votre pseudo est déjà utilisé par un membre";
        $i++;
    }

    if (strlen($pseudo) < 3 || strlen($pseudo) > 15)// vérification de si le pseudo n'est pas tros grand
    {   //ou trop petit      //si oui on envoie un message de erreur
    ...
    {         //si il y a un probleme on envoie un message  
